...
use constant { FAIL_ON_DUPLICATE => 0, IGNORE_DUPLICATES => 1,
    RENAME_DUPLICATES => 2 };
my @DEFAULT_CONFIG_LOCATIONS = (
    $FindBin::Bin . "/ngs-archive-linker.conf",
    $ENV{HOME} . "/.irida/ngs-archive-linker.conf",
    "/etc/irida/ngs-archive-linker.conf"
);

my (
    $baseURL,       $projectId,        $fileType,
    @sampleIds,     $directory,        $help,
    $verbose,       $vverbose,         $username,
    $password,      $ignoreDuplicates, $renameDuplicates,
    $flatDirectory, $download,         $configFile
);
...
if ( !defined $projectId ) {
    print
"Error: Project ID must be defined.  Use --help option for argument details.\n";
    exit(1);
}
...
if ( !$baseURL ) {
    $baseURL = $config->param("apiurls.BASEURL")
      or die "Base URL not set and cannot be read from config file";
}
...
my $tokenstr =
  getToken( $baseURL, $username, $password, $client_id, $client_secret );
...
sub createLinks {
    ...
    if ( keys %$samples > 0 ) {
        ...
        if ( !-d $projectDir ) {
            make_path($projectDir)
              or die "Couldn't make project directory $projectDir: $!";
        ...
        foreach my $sampleId ( keys %$samples ) {
            my $sampleDir;
            if ( !$flatDirectory )
            { #if we don't want flat directorys, create a directory for the sequence file's sample
                $sampleDir = "$projectDir/$sampleId";
                if ( !-d $sampleDir ) {
                    make_path($sampleDir)
                      or die "Couldn't make sample directory $sampleDir: $!";
            ...
            loopFileType( $samples->{$sampleId}, "fastq", $download,
                $sampleDir, $duplicateLevel, $client, $headers );
            loopFileType( $samples->{$sampleId}, "assembly", $download,
                $sampleDir, $duplicateLevel, $client, $headers );
        }

        if ( $fileCount > 0 ) {
            print
"Created $fileCount files for $sampleCount samples in $projectDir\n";
        }
...
sub loopFileType {
    ...
    foreach my $fileref ( @{ $sample->{$type} } ) {
        ...
        if ($download) {
            ...
            if ( !-e $filename ) {
                die
"Error: Script cannot see a file to be linked: $filename, but you can still download it with the --download option. If the problem persists ensure you have access to the sequence files and assemblies directory.";
            }
...
sub checkFileExistence {
    ...
    if ( ( -l $newfile or -e $newfile )
        && $duplicateLevel == FAIL_ON_DUPLICATE )
    {
        print "Error: File $newfile already exists\n";
        print
"To skip already existing files, use --ignore option.  To create new files with a unique name, use --rename option.\n";
        exit(1);
    }
    elsif ( ( -l $newfile or -e $newfile )
        && $duplicateLevel == IGNORE_DUPLICATES )
    {
...
sub linkFile {
    ...
    if ($writeFile) {
        symlink( $filename, $writeFile )
          or die "Couldn't create link to file $filename: $!";
...
sub getSampleUrlsFromList {
    ...
    foreach my $id (@$sampleIds) {
        my ( $sequencerId, $sampleUrl ) =
          getSampleDetails( $project, $id, $agent, $headers );
...
sub getSampleDetails {
    ...
    my $url =
      getRelFromLinks( $project->{resource}->{links}, 'project/samples' );
    $url = "$url/$sampleId";
...
"Error: Server returned internal server error.  You may have used an incorrect URL for the API.\n";
        ...
        print
          "Error: This user does not have access to the resource at $url.\n";
        exit(1);
    }
        ...
        print
"Error: Server returned status code $code when requesting resource $url.\n";
        exit(1);
    }
}
